...
VolumeHandle* DcmtkVolumeReader::readDicomFiles(const vector<string> &fileNames,
...
*/
    ...
    case 32:
        ...
        try {
            VolumeUInt8* dataset2 = new VolumeUInt8(tgt::ivec3(dx_, dy_, dz_), 8);
    ...
    VolumeHandle* vh = new VolumeHandle(dataset, tgt::vec3(x_spacing, y_spacing, z_spacing) * 0.01f, tgt::vec3(0.0f));
...
VolumeHandle* DcmtkVolumeReader::readDICOMDIR(const string &fileName,
                                        const string &filterSeriesInstanceUID)
{
...
VolumeHandle* DcmtkVolumeReader::readDicomFile(const string &fileName) {
    vector<string> files;
    ...
    throw (tgt::FileException, std::bad_alloc)
{
    ...
    /*else if (fileName.find("dicom://") == 0) {
        ...
        if (volume)
            volume->setOrigin(VolumeOrigin("dicom", fileName));
    } */
    ...
    if (volume) {
        ...
        volume->setOrigin(VolumeOrigin("dicom", fileName, (!seriesInstanceUID.empty() ? "SeriesInstanceUID=" + seriesInstanceUID : "")));
...
VolumeHandle* DcmtkVolumeReader::read(const std::vector<std::string> &fileNames) {
    dx_ = dy_ = dz_ = 0;
    return readDicomFiles(fileNames);
}

VolumeHandleBase* DcmtkVolumeReader::read(const VolumeOrigin& origin) 
    throw (tgt::FileException, std::bad_alloc) 
{
    VolumeCollection* collection = read(origin.getURL());
    tgtAssert(collection && !collection->empty(), "exception expected");
    VolumeHandleBase* result = collection->first();
    delete collection;
    return result;
}
...
std::vector<VolumeOrigin> DcmtkVolumeReader::listVolumes(const std::string& url) const 
    throw (tgt::FileException) 
{
    ...
    // delegate to listSeries() and convert returned DcmtkSeriesInfos to VolumeOrigins
    ...
    for (size_t i=0; i<seriesInfos.size(); i++) {
        ...
        if (hour > -1 && minute > -1 && second > -1) {
            DateTime time = DateTime::createTime(hour, minute, second);
