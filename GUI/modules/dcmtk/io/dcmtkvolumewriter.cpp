...
#include "modules/dcmtk/io/dcmtkvolumewriter.h"
...
void DcmtkVolumeWriter::write(const std::string& fileName, const VolumeHandleBase* volumeHandle)
    throw (tgt::IOException) {

    if (!volumeHandle || !volumeHandle->getRepresentation<Volume>()) {
        LERROR("Empty volume handle passed");
        return;
    }

    DcmtkVolumeDescription desc;
    DcmtkVolumeWriter::writeDicomFiles(desc, volumeHandle->getRepresentation<Volume>(), fileName);
}

// static member functions

bool DcmtkVolumeWriter::writeDicomFiles(const DcmtkVolumeDescription &desc, const Volume* volume,
...
bool DcmtkVolumeWriter::writeDicomFile(const DcmtkVolumeDescription &desc, const Volume* volume,
    ...
    if (const VolumeUInt8* v = dynamic_cast<const VolumeUInt8*>(volume))
        voxels = (char*)v->voxel();
    else if (const VolumeUInt16* v = dynamic_cast<const VolumeUInt16*>(volume))
        voxels = (char*)v->voxel();
