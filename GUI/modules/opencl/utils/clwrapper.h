...
class VRN_MODULE_OPENCL_API Program {
public:
...
public:
    ...
    size_t getSize() const { return tex_->getSizeOnGPU(); }
...
public:
    ///flags: CL_MEM_READ_WRITE CL_MEM_WRITE_ONLY CL_MEM_READ_ONLY CL_MEM_USE_HOST_PTR CL_MEM_ALLOC_HOST_PTR CL_MEM_COPY_HOST_PTR
    ImageObject3D(const Context* context, cl_mem_flags flags, const Volume* vol);
...
struct VolumeWriteBuffer {
    ...
    VolumeWriteBuffer(const Context* context, Volume* vol);
...
VolumeWriteBuffer createVolumeWriteBuffer(const Context* context, Volume* vol);
