...
void DynamicCLProcessor::generateVolumeRepresentations() {
    ...
    for(size_t i = 0; i < curArgs_.size(); i++) {
        if(curArgs_.at(i).attType_ == IMAGE3D && curArgs_.at(i).att_ == INPORT) {
            ...
            const VolumeHandleBase* handle  = ((VolumePort*)(getPort(curArgs_.at(i).values_["name"])))->getData();
            if(handle)
                volObj = new ImageObject3D(context_, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, handle->getRepresentation<Volume>());
        ...
        if(curArgs_.at(i).attType_ == IMAGE3D && curArgs_.at(i).att_ == OUTPORT) {
            ...
            Volume* vol = generateVolumeFromTags(curArgs_.at(i).values_["volumedimensions"], curArgs_.at(i).values_["volumetype"]);
            ...
            if(vol) {
                ...
                VolumeHandle* vh = new VolumeHandle(vol, tgt::vec3(1.f), tgt::vec3(0.0f));
                vp->setData(vh);
                // TODO: support correct spacing or copying volume information from an inport
...
Volume* DynamicCLProcessor::generateVolumeFromTags(const std::string& dims, const std::string& type) {
    ...
    // TODO: signed types
    if(lowerType == "uint8")
        return new VolumeUInt8(volumeDims);
    else if(lowerType == "uint16")
        return new VolumeUInt16(volumeDims);
    //else if(lowerType == "uint32")
        //return new VolumeUInt32(volumeDims);
    else if(lowerType == "3xuint8")
        return new Volume3xUInt8(volumeDims);
    else if(lowerType == "3xuint16")
        return new Volume3xUInt8(volumeDims);
    else if(lowerType == "4xuint8")
        return new Volume4xUInt8(volumeDims);
    else if(lowerType == "4xuint16")
        return new Volume4xUInt16(volumeDims);
    else if(lowerType == "float")
        return new VolumeFloat(volumeDims);
    else if(lowerType == "4xfloat")
        return new Volume4xFloat(volumeDims);
    else {
...
void DynamicCLProcessor::process() {
    ...
    //if(curKernelWorkDims_.x <= 1 || curKernelWorkDims_.y <=1 || curSharedTexs_.empty()) {
        //LWARNING("No connected image ports detected for output.  Skipping execution.");
        //return;
    //}
    ...
    if (kernel) {
        ...
        for(size_t i = 0; i < curArgs_.size(); i++) {
            if (curArgs_.at(i).attType_ == IMAGE2D) {
                ...
                if(shrTex)
                    ...
                    LWARNING("Image port '" << curArgs_.at(i).values_["name"] << "' not connected.");
                ...
                if(volTex)
                    ...
                    LWARNING("Volume port '" << curArgs_.at(i).values_["name"] << "' not connected.");
                volTexCounter++;
                ...
                if(volBuf) {
                    ...
                    LWARNING("Volume port '" << curArgs_.at(i).values_["name"] << "' not initialized with Volume.");
                volBufCounter++;
        ...
        if(curWorkDimensionSource_->att_ == INPORT || curWorkDimensionSource_->att_ == OUTPORT) {
            if(curWorkDimensionSource_->attType_ == IMAGE2D) {
                ...
                const VolumeHandleBase* h = ((VolumePort*)getPort(wdName))->getData();
                if(!h) {
...
void DynamicCLProcessor::parseArguments(const std::string& programSource) throw(VoreenException) {
    ...
    // delete ordinary comments  TODO: Fix a bug that prevents comments of the form "//$ tag=value // other comments" to be processed correctly
    ...
    while(commentPos != std::string::npos) {
        ...
        args.erase(commentPos, lineEnd - commentPos + 1);
...
Property* DynamicCLProcessor::generateNewProperty(ArgInfo& arg) throw (VoreenException) {
    ...
    if(arg.attType_ == TRANSFUNCPROPERTY) {
        ...
        ((TransFuncProperty*)p)->set(new TransFuncIntensity());
    }
