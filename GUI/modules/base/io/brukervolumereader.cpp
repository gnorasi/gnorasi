...
VolumeHandleBase* BrukerVolumeReader::read(const VolumeOrigin& origin)
    ...
    if (!error) {
        ...
        for (int frame = start; frame < end; ++frame) {
            ...
            if (vol) {
                ...
                VolumeHandle* vh = new VolumeHandle(vol, sliceThickness, vec3(0.0f));

                //if (recoTranspose == 1) {
                    //VolumeHandle* vhTransposed = VolumeOperatorTranspose::APPLY_OP(vh, 0, 1);
                    //delete vh;
                    //vh = vhTransposed;
                //}
                //else if (recoTranspose == 2) {
                    //VolumeHandle* vhTransposed = VolumeOperatorTranspose::APPLY_OP(vh, 1, 2);
                    //delete vh;
                    //vh = vhTransposed;
                //}
                //else if (recoTranspose == 3) {
                    //VolumeHandle* vhTransposed = VolumeOperatorTranspose::APPLY_OP(vh, 0, 2);
                    //delete vh;
                    //vh = vhTransposed;
                //}
...
bool BrukerVolumeReader::mapTokenToMetaData(const std::string& voreenKey, VolumeHandle* vh, const JCampToken& t) {
    switch(t.getType()) {
        case JCampToken::STRING:
        case JCampToken::ENUM:
...
bool BrukerVolumeReader::mapMetaData(const std::string& key, const std::string& voreenKey, VolumeHandle* vh, const JCampParser& parser, size_t id, size_t numFrames) {
...
std::vector<VolumeOrigin> BrukerVolumeReader::listVolumes(const std::string& url) const 
        throw (tgt::FileException)
{
    ...
    for(size_t i=0; i<sliceCols.size(); i++) {
        VolumeOrigin origin("bruker", fileName);
