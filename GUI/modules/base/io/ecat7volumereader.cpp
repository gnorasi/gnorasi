...
VolumeHandleBase* ECAT7VolumeReader::read(const VolumeOrigin& origin)
    throw (tgt::FileException, std::bad_alloc)
{
    ...
    if (collection && collection->size() == 1) {
        ...
        while(!collection->empty()) {
           VolumeHandleBase* vh = collection->first();
    ...
    throw(tgt::CorruptedFileException, tgt::IOException, std::bad_alloc)
{
    ...
    for(size_t i=0; i<s.subVolumes_.size(); i++) {
        ...
        VolumeHandle* vh = new VolumeHandle(vol, spacing * 0.1f, offset * 0.1f);
...
std::vector<VolumeOrigin> ECAT7VolumeReader::listVolumes(const std::string& url) const 
        throw (tgt::FileException)
{
    ...
    for(size_t i=0; i<s.subVolumes_.size(); i++) {
        VolumeOrigin origin("ecat7", fileName);
