...
public:
    ...
    std::vector<VolumeOrigin> listVolumes(const std::string& url) const
        throw (tgt::FileException);
protected:
    std::vector<SliceCollection> listSliceCollections(const std::string& url) const
        throw (tgt::FileException);
    bool mapTokenToMetaData(const std::string& voreenKey, VolumeHandle* vh, const JCampToken& t);
    bool mapMetaData(const std::string& key, const std::string& voreenKey, VolumeHandle* vh, const JCampParser& parser, size_t id, size_t numFrames);
