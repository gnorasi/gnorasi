...
#include "interfilevolumereader.h"
    ...
    throw (tgt::CorruptedFileException, tgt::IOException, std::bad_alloc)
{
    ...
    if (!error) {
        ...
        VolumeCollection* volumeCollectionMirrored = new VolumeCollection();
        for (size_t i = 0; i < volumeCollection->size(); ++i) {
            VolumeHandleBase* volume = volumeCollection->at(i);
            if (volume == 0)
                continue;
            ...
            //
            //volume = volume->clone();
            VolumeHandle* t = VolumeOperatorMirrorZ::APPLY_OP(volume);
            delete volume;
            volume = t;
            volumeCollectionMirrored->add(volume);
        }
        delete volumeCollection;
        return volumeCollectionMirrored;
    }
    else
