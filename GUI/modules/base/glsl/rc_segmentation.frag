...
uniform VOLUME_STRUCT segmentationParameters_;  // segmentation volume
...
vec4 applySegmentationClassification(vec3 sampleVal, vec4 voxel, VOLUME_STRUCT segmentationParameters) {
    ...
    float segValue = getVoxel(segmentationParameters, sampleVal).a;
...
void rayTraversal(in vec3 first, in vec3 last) {
    ...
    RC_BEGIN_LOOP {
        ...
        // calculate gradients
        voxel.xyz = RC_CALC_GRADIENTS(voxel.xyz, samplePos, volumeStruct_, t, rayDirection, entryPoints_, entryParameters_);
        ...
        color.rgb = RC_APPLY_SHADING(voxel.xyz, samplePos, volumeStruct_, color.rgb, color.rgb, color.rgb);
