...
uniform VOLUME_STRUCT segmentationParameters_;  // segmented dataset
...
vec4 fillIDBuffer(in vec4 entry, in vec4 firstHitPoint, in vec4 exit) {
    ...
    vec4 result;
    float seg = textureLookup3D(segmentationParameters_, firstHitPoint.rgb).a;
    ...
    if (seg == 0.0) {
        ...
        while ((seg == 0.0) && (t_add < penetrationDepth_)) {
            ...
            seg = textureLookup3D(segmentationParameters_, pos).a;
        }
        result.rgb = pos;
    }
    if (seg > 0.0)
