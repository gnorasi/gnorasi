...
uniform VOLUME_STRUCT volumeStruct_;    // texture lookup parameters for volume_
...
vec4 getFirstHitColor(in vec3 firstHitPos) {
    ...
    // calculate gradients
    voxel.xyz = RC_CALC_GRADIENTS(voxel.xyz, firstHitPos, volume_, volumeStruct_, 0.0, vec3(0.0), entryPoints_);

    // apply classification
    result = RC_APPLY_CLASSIFICATION(transferFunc_, voxel);

    // apply shading
    result.rgb = RC_APPLY_SHADING(voxel.xyz, firstHitPos, volumeStruct_, result.rgb, result.rgb, result.rgb);
