...
std::string MultiVolumeRaycaster::generateHeader() {
    ...
    /*
    This define needs to be set as the VOLUME_STRUCT uniforms are not valid to use in the mod_sampler3D shader, when multiple volumes are used.
    This is a problem only on Nvidia Driver version larger than 295.73, tested to 296.10.
    */
    if(count > 1)
        headerSource += "#define IGNORE_VOLUME_STRUCT_UNIFORMS_WHEN_SAMPLING\n";
    ...
    headerSource += "#define RC_APPLY_SHADING_1(gradient, samplePos, volumeStruct, ka, kd, ks) ";
    ...
    headerSource += "#define RC_APPLY_SHADING_2(gradient, samplePos, volumeStruct, ka, kd, ks) ";
    ...
    headerSource += "#define RC_APPLY_SHADING_3(gradient, samplePos, volumeStruct, ka, kd, ks) ";
    ...
    headerSource += "#define RC_APPLY_SHADING_4(gradient, samplePos, volumeStruct, ka, kd, ks) ";
