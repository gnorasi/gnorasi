...
SliceViewer::SliceViewer()
    ...
    , mousePositionProperty_("mousePositionProperty", "Mouse Position", tgt::vec3(0.f), tgt::vec3(-1.f), tgt::vec3(1.f))
    ...
    , lastPickingPosition_(-1, -1, -1)
{
    ...
    mousePositionProperty_.setWidgetsEnabled(false);
    addProperty(mousePositionProperty_);
    ...
    mousePositionProperty_.setGroupID("informationOverlay");
...
void SliceViewer::updateSliceProperties() {
    ...
    if (inport_.getData() && inport_.getData()->getRepresentation<Volume>())
        volumeDim = inport_.getData()->getRepresentation<Volume>()->getDimensions();
...
void SliceViewer::process() {
    ...
    if (texMode_.isSelected("2d-texture")) {        // 2D texture
        float bitScale = 1.f;
        if (dynamic_cast<const VolumeUInt16*>(volume) && volume->getBitsStored() == 12)
            bitScale = 16.f; //< scale factor for 12 bit volumes stored in 16 bit
        ...
        ++pos, x = pos % numSlicesCol, y = pos / numSlicesCol)
    {
        ...
        if (texMode_.isSelected("2d-texture")) {
            ...
            if (sliceTexture) {
                bindSliceTexture(sliceTexture, &texUnit);
...
void SliceViewer::renderInfoTexts() const {
    ...
    if (!showCursorInfos_.isSelected("never")) {
        ...
        if (lastPickingPosition_.x != -1) {
            ...
            oss << volume->getVoxelValueAsString(tgt::svec3(lastPickingPosition_)); 
            renderStr = oss.str(); 
...
tgt::vec3 SliceViewer::screenToVoxelPos(tgt::ivec2 screenPos) const {
    ...
    if (!inport_.getData() || !inport_.getData()->getRepresentation<Volume>() || !outport_.getRenderTarget())
        return tgt::vec3(-1.f);

    tgt::vec3 volumeDim(inport_.getData()->getRepresentation<Volume>()->getDimensions());
...
tgt::mat4 SliceViewer::generatePickingMatrix() const {
    ...
    if (!inport_.hasData() || !inport_.getData()->getRepresentation<Volume>())
    ...
    if (showCursorInfos_.isSelected("onMove") ||
        ...
        (e->getEventType() == tgt::MouseEvent::MOUSEPRESSEVENT || e->getEventType() == tgt::MouseEvent::WHEELEVENT))) ) {
            ...
            if (mousePosition_ != e->coord()) {
                ...
                tgt::vec3 pos = screenToVoxelPos(e->coord());

                const Volume* volume = inport_.getData()->getRepresentation<Volume>();
                tgt::ivec3 volDim = volume->getDimensions();
                pos = pos / tgt::vec3(volDim);
                pos = clamp(pos, tgt::vec3(0.f), tgt::vec3(1.f));
                pos -= tgt::vec3(0.5f);
                pos *= 2.f;
                mousePositionProperty_.set(pos);
                invalidate();
            }
    }
    else {
...
void SliceViewer::shiftEvent(tgt::MouseEvent* e) {
    ...
    tgt::vec3 volDim = tgt::vec3(inport_.getData()->getRepresentation<Volume>()->getDimensions()) - 1.f;
