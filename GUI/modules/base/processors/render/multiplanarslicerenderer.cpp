...
void MultiplanarSliceRenderer::renderSlice(SliceAlignment sliceAlign, int sliceNo, tgt::TextureUnit& texUnit) {
    ...
    const VolumeHandleBase* volume =  inport_.getData();
...
void MultiplanarSliceRenderer::updateNumSlices() {
    ...
    tgtAssert(inport_.hasData() && inport_.getData()->getRepresentation<Volume>(), "No volume");
    tgt::ivec3 volDim = inport_.getData()->getRepresentation<Volume>()->getDimensions();
