...
void ExplosionProxyGeometry::process() {
    tgtAssert(inportVolume_.getData()->getRepresentation<Volume>(), "no volume");
...
void ExplosionProxyGeometry::doBricking() {
    ...
    const VolumeHandleBase* volume = inportVolume_.getData();
...
tgt::vec3 ExplosionProxyGeometry::getSliceWidth(const VolumeHandleBase* volume) {
    tgt::vec3 sliceWidth;
...
void ExplosionProxyGeometry::updatePlaneWidgets() {
    if (brickingMode_.get() == "regularGrid" || brickingMode_.get() == "manualGrid" || selectedBricks_.empty()) {
        tgt::ivec3 numSlices = inportVolume_.getData()->getRepresentation<Volume>()->getDimensions();
