...
const std::string Labeling::setLabelingWidget_("labelingWidget");
const std::string Labeling::setLayout_("layout");
const std::string Labeling::setLabelColor_("labelColor");
const std::string Labeling::setHaloColor_("haloColor");
const std::string Labeling::setFont_("font");
const std::string Labeling::setFontSize_("fontSize");
const std::string Labeling::switchDrawHalo_("drawHalo");
const std::string Labeling::setSegmentDescriptionFile_("segmentDescriptionFile");
...
Labeling::Labeling()
    ...
    , labelFile_("labelFile", "Segment Description File", "Path to Segment Description File",
        VoreenApplication::app()->getDataPath(""))
...

{
    ...
    labelFile_.onChange(CallMemberAction<Labeling>(this, &Labeling::labelFileChanged));
...
void Labeling::initialize() throw (tgt::Exception) {
#ifdef VRN_MODULE_FONTRENDERING
...
void Labeling::deinitialize() throw (tgt::Exception) {
...
void Labeling::appendUnsegmentedLabels() {
...
void Labeling::calcLabelPositions() {
...
void Labeling::toWorld(Label* pLabel) {
...
void Labeling::connPointFromLabelPos(const Label &pLabel, ivec2 &connPoint, ivec2 &normal, bool calcNormal) {
...
void Labeling::labelPosFromConnPoint(const Label &pLabel, ivec2 &labelPos) {
...
bool Labeling::resolveLineIntersections(LabelVec &pLabels) {
...
void Labeling::lineIntersection(ivec2 P1, ivec2 Q1, ivec2 P2, ivec2 Q2,
...
void Labeling::drawHalo(labeling::Bitmap<GLfloat> &bitmapParam, int width, int height,
...
bool Labeling::intersectBoxHull(ivec2 const &bl, ivec2 const &tr) {
...
bool Labeling::pointInBox(ivec2 const &bl, ivec2 const &tr, ivec2 const &point) {
...
Labeling::Circle Labeling::generateCircle(float radius) {
    Circle circle;
    CirclePixel pixel;
...
Labeling::CirclePixel Labeling::getNextCirclePixel(int id, Circle circle, ivec2 curPoint,
...
bool Labeling::findLabelPathOne(const Label &pLabel, float step, float initAngle, float initMaxStepAngle,
...
bool Labeling::findBezierPoints(Label &pLabel) {
    // -------------------------------------------------
...
bool Labeling::checkPatchQuality(Label pLabel) {
...
void Labeling::placeOffLabels(LabelVec &offLabels) {
    const int H_GAP = 10;
    const int V_GAP = 8;
...
void Labeling::addUnsegmentedLabelData(std::string text) {
...
//  </segmentation>
//
bool Labeling::readSegmentData(std::string filename) {
    ...
    if (xmlDoc_.LoadFile(filename.c_str())) {
        ...
        LERROR("Failed to load segment description file \"" << filename << "\"");
        return false;
    }
...
void Labeling::labelFileChanged() {
    if (!labelFile_.get().empty()) {
...
void Labeling::createFilterKernels() {
    // no filter
...
void Labeling::writeDistanceMapToFile() {
    std::ofstream myfile;
...
void Labeling::mouseReleaseEvent(MouseEvent* e) {
...
void Labeling::mouseMoveEvent(MouseEvent* e) {
    e->ignore();
