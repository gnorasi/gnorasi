...
#include "meshclippingwidget.h"

namespace voreen {

const float MeshClippingWidget::EPS(0.000001f);

MeshClippingWidget::MeshClippingWidget()
...
void MeshClippingWidget::initialize() throw (tgt::Exception) {
    GeometryRendererBase::initialize();
}

Processor* MeshClippingWidget::create() const {
    return new MeshClippingWidget();
}

void MeshClippingWidget::setIDManager(IDManager* idm) {
    if (idManager_ == idm)
        return;
...
void MeshClippingWidget::planeManipulation(tgt::MouseEvent* e) {
    e->ignore();
...
void MeshClippingWidget::renderPicking() {
    if (!idManager_)
        return;
...
void MeshClippingWidget::render() {
...
tgt::vec3 MeshClippingWidget::getLlf() {
    return inport_.getData()->getLLF();
}

tgt::vec3 MeshClippingWidget::getUrb() {
    return inport_.getData()->getURB();
}

void MeshClippingWidget::paintAnchor(const Anchor& anchor) {
...
void MeshClippingWidget::paintLine(const Line& line) {
...
inline float MeshClippingWidget::getPointLineDistance(const tgt::vec3& point, const tgt::vec3& linePositionVector, const tgt::vec3& lineDirectionVector) const {
    return tgt::length(tgt::cross(lineDirectionVector, point - linePositionVector)) / tgt::length(lineDirectionVector);
}

inline float MeshClippingWidget::getPointLineProjectionScalar(const tgt::vec2& point, const tgt::vec2& linePositionVector, const tgt::vec2& lineDirectionVector) const {
    return tgt::dot(point - linePositionVector, lineDirectionVector) / tgt::lengthSq(lineDirectionVector);
}

bool MeshClippingWidget::getIntersectionPointInAABB(tgt::vec3& intersectionPoint, const tgt::vec3& linePositionVector, const tgt::vec3& lineDirectionVector, const tgt::vec4& plane) {
...
void MeshClippingWidget::addAnchor(const tgt::vec3& linePositionVector, const tgt::vec3& lineDirectionVector, const tgt::vec4& plane) {
...
void MeshClippingWidget::addLine(const tgt::vec4& planeAABB) {
    Anchor* begin = 0;
    Anchor* end = 0;
...
void MeshClippingWidget::updateAnchorsAndLines() {
