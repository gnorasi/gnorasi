...
void SegmentationValidation::computeMetrics() {
    const VolumeHandleBase* segHandle = inportSegmentation_.getData();
    const VolumeHandleBase* refHandle = inportReference_.getData();
    tgtAssert(segHandle && refHandle, "No input data");

    const Volume* segVolume = segHandle->getRepresentation<Volume>();
    const Volume* refVolume = refHandle->getRepresentation<Volume>();
...
size_t SegmentationValidation::getNumCommonVoxels(const Volume* volumeA, const Volume* volumeB) {
