...
void VolumeRegistration::process() {
    ...
    if (!enableProcessing_.get()) {
        outport_.setData(const_cast<VolumeHandleBase*>(inportVolume_.getData()), false);
        ...
        VolumeHandleBase* outputVolume = new VolumeHandleDecoratorReplace(inportVolume_.getData(), "transformation", new Mat4MetaData(registrationMatrix));

        outport_.setData(outputVolume);
    }
}

void VolumeRegistration::initialize() throw (tgt::Exception) {
...
tgt::mat4 VolumeRegistration::computeRigidBodyTransformation(const VolumeHandleBase* inputHandle, const VolumeHandleBase* refHandle,
        tgt::vec3 p0, tgt::vec3 p0Ref, tgt::vec3 p1, tgt::vec3 p1Ref, tgt::vec3 p2, tgt::vec3 p2Ref, bool ignoreReferenceTrafoMatrix) {

    tgtAssert(inputHandle && inputHandle->getRepresentation<Volume>(), "no volume");
    tgtAssert(refHandle && refHandle->getRepresentation<Volume>(), "no volume");
...
#ifdef VRN_MODULE_JAMA
tgt::mat4 VolumeRegistration::computeAffineTransformation(const VolumeHandleBase* inputHandle, const VolumeHandleBase* refHandle,
...
#else
tgt::mat4 VolumeRegistration::computeAffineTransformation(const VolumeHandleBase* /*inputHandle*/, 
        const VolumeHandleBase* /*refHandle*/, std::vector<vec3> /*inputPoints*/, 
        std::vector<vec3> /*refPoints*/, bool /*ignoreReferenceTrafoMatrix*/) {
    
    LERROR("affine transformation requires the JAMA module");
    return tgt::mat4::identity;
}
#endif // VRN_MODULE_JAMA


vec3 VolumeRegistration::transformFromVoxelToVolumeCoords(vec3 point, const VolumeHandleBase* vol) {
