...
const std::string MultiSurfaceRenderer::loggerCat_("voreen.hpmc.MultiSurfaceRenderer");
...
void MultiSurfaceRenderer::initialize() throw (tgt::Exception) {
...
void MultiSurfaceRenderer::compile(const VolumeHandleBase* volumeHandle) {
...
std::string MultiSurfaceRenderer::generateHeader(const VolumeHandleBase* volumeHandle) {
    std::string headerSource = "";
    tgt::ivec3 dimensions = tgt::ivec3(128,128,128);
    GLint volID = 0;
    if (volumeHandle) {
        dimensions = volumeHandle->getRepresentation<Volume>()->getDimensions();
