...
const std::string MarchingCubesRenderer::loggerCat_("voreen.hpmc.MarchingCubesRenderer");
...
void MarchingCubesRenderer::initialize() throw (tgt::Exception) {
    GeometryRendererBase::initialize();

    loadShader();

    if (!mcExtractPrg_ || !mcRenderPrg_) {
...
void MarchingCubesRenderer::deinitialize() throw (tgt::Exception) {
...
void MarchingCubesRenderer::compile(const VolumeHandleBase* volumeHandle) {
...
std::string MarchingCubesRenderer::generateExtractHeader(const VolumeHandleBase* volumeHandle) {
    std::string headerSource = GeometryRendererBase::generateHeader();

    tgt::ivec3 volDim = tgt::ivec3(128);
    GLint volID = 0;
    if (volumeHandle) {
        volDim = volumeHandle->getRepresentation<Volume>()->getDimensions();
        volID = volumeHandle->getRepresentation<VolumeGL>()->getTexture()->getId();
    }
...
void MarchingCubesRenderer::process() {
    ...
    tgtAssert(volumeInport_.getData()->getRepresentation<Volume>(), "No volume");
    ...
    mcExtractPrg_->setUniform("volumeCubeSize_", volumeInport_.getData()->getCubeSize());
    mcExtractPrg_->setUniform("volumeOffset_", volumeInport_.getData()->getOffset());

    // bind volume texture
    TextureUnit volUnit;
    volUnit.activate();
    volumeInport_.getData()->getRepresentation<VolumeGL>()->getTexture()->bind();
