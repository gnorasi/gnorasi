...
void PythonEditor::createWidgets() {
    ...
    decreaseFontSizeBt_->setIcon(QIcon(":/icons/viewmag_.png"));
...
void PythonEditor::newScript() {
    ...
    if (PythonModule::getInstance() && PythonModule::getInstance()->isInitialized()) {
        script_ = PythonModule::getInstance()->load("template.py", false);
        if (script_) {
...
void PythonEditor::openScript() {
    ...
    if (!fileName.isEmpty()) {
        ...
        script_ = PythonModule::getInstance()->load(fileName.toStdString(), false);
        if (!script_) {
