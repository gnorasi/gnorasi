...
void VolumeCreateBase::fillPlane(VolumeUInt8* vds, vec3 center, vec3 normal, uint8_t value) {
...
void VolumeCreateBase::fillCircle(VolumeUInt8* vds, vec3 center, float radius, uint8_t value) {
    for (size_t voxel_y = 0; voxel_y < vds->getDimensions().y; ++voxel_y) {
        for (size_t voxel_x = 0; voxel_x < vds->getDimensions().x; ++voxel_x) {
...
void VolumeCreateBase::fillOrientedCircle(VolumeUInt8* vds, vec3 center, vec3 normal, float radius, uint8_t value) {
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
        for (size_t voxel_y=0; voxel_y<vds->getDimensions().y; voxel_y++) {
            for (size_t voxel_x=0; voxel_x<vds->getDimensions().x; voxel_x++) {
                vec3 pos = static_cast<vec3>(tgt::svec3(voxel_x, voxel_y, voxel_z));
...
void VolumeCreateBase::fillSphere(VolumeUInt8* vds, vec3 center, float radius, uint8_t value) {
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
        for (size_t voxel_y=0; voxel_y<vds->getDimensions().y; voxel_y++) {
            for (size_t voxel_x=0; voxel_x<vds->getDimensions().x; voxel_x++) {
...
void VolumeCreateBase::fillEllipsoid(VolumeUInt8* vds, vec3 center, vec3 radius, uint8_t value) {
...
void VolumeCreateBase::fillBox(VolumeUInt8* vds, ivec3 start, ivec3 end, uint8_t value) {
    ivec3 i;
...
void VolumeCreateBase::fillOrientedBox(VolumeUInt8* vds, vec3 center, vec3 dir, float lengthA, float lengthB,
...
void VolumeCreateBase::fillCone(VolumeUInt8* vds, vec3 center, vec3 /*dir*/, float /*lengthC*/, float yStart, float yEnd, float topRadius, float bottomRadius, uint8_t value) {
    //dir = normalize(dir);
    //center.y = 0.0f;
    //vec3 dir2 = cross(dir, vec3(0.0f, 1.0f, 0.0f));
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
...
void VolumeCreateBase::applyPerturbation(Volume* /*vds*/, ivec3 /*dimensions*/, vec3 /*frequency*/, vec3 /*amplitude*/) {
