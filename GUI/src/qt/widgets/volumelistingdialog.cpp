...
void VolumeListingDialog::setOrigins(const std::vector<VolumeOrigin>& origins, VolumeReader* reader) {
    origins_ = origins;
    reader_ = reader;

    titleLabel_->setText(tr("The selected file contains %1 volumes. Please select the volumes to load:").arg(origins.size()));
...
std::vector<VolumeOrigin> VolumeListingDialog::getFilteredOrigins(QString filterStr, QString filterAttribute /*= ""*/) const {
    if (filterStr == "")
        return origins_;
    ...
    for (size_t i=0; i<origins_.size(); i++) {
        const VolumeOrigin& origin = origins_.at(i);
        bool match = false;
        if (filterAttribute == "") {
