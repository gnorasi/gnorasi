...
void ItiOtbImageViewerPanel::initialize(){
    ...
    connect(m_pSetupTab,SIGNAL(colorChanged()),this,SLOT(applyChangeColorClass()));
...
void ItiOtbImageViewerPanel::applyChangeColorClass(){
    Command *pCommand = m_commandHash[(int)SLOT_CL_CHANGECOLOR];
    if(pCommand)
        pCommand->execute();
}
//!
