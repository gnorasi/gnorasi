...
import gr.iti.gnorasi.service.rest.GeoRulesServiceREST;
...
public class RuleFire {
	...
	private static Logger logger = Logger.getLogger(RuleFire.class);
	...
	public Map<URI,Literal> fire(GeoRule rule, DataSource ds, RepositoryConnection con ) {
		...
		for (GeoRulePredicate grp : rule.getPredicates()) {
			if (firstPredicate) {
				uris = geoQuery(grp, rule.getHead(), con);
	...
	public Map<URI,Literal> geoQueryAssigned(GeoRulePredicate grp, GeoRuleHead headClause, RepositoryConnection con) {
		...
		try {
			...
			String reg2Query = "SELECT ?reg2 ?reg2WKT WHERE {\n" +
									...
									"?dep2 gno:"+Constants.depictsObject+" ?obj2 . \n" +
									"?obj2 rdf:type <"+headClause.getRegionDepiction()+"> .\n" +
									"FILTER (?reg != ?reg2) . \n" +
									"FILTER NOT EXISTS {" +
										"?reg2 gno:"+Constants.depicts+" ?dep22 . " +
										"?dep22 gno:"+Constants.depictsObject+" ?obj22 . " +
										"?obj22 rdf:type <"+grp.getRegionDepiction()+"> ." +
									"} . \n"+
	...
	public Map<URI,Literal> geoQuery(GeoRulePredicate grp, GeoRuleHead headClause, RepositoryConnection con) {
		...
		try {
			...
			String reg2Query = "SELECT ?reg2 ?reg2WKT WHERE {\n" +
									...
									segmentationLevelRestriction(grp, "reg2") + //that belong the seg. level
									//imageRestriction(grp, "reg2")+ //that belong to the image specified in the rule
									superClassRestriction(headClause, "reg2", con) + // that are rdf:type of rule head superclass
									"FILTER (?reg != ?reg2) . \n" +
									"FILTER NOT EXISTS {" +
										"?reg2 gno:"+Constants.depicts+" ?dep2 . " +
										"?dep2 gno:"+Constants.depictsObject+" ?obj2 . " +
