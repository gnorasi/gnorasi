...
public class RuleFireAssignedRegions {

	private static Logger logger = Logger.getLogger(RuleFireAssignedRegions.class);
	...
	public void fireAll(List<GeoRule> rules, DataSource ds, RepositoryConnection con, URI context) {
		...
		for (GeoRule rule : rules) {
			try {
				...
				List<URI> fuzzyAssignments = getAssignedList(rule.getHead(), con);
				URI region = null;
				...
				if (uris != null && region != null) {
					for (URI entry : uris.keySet()) {
						if (fuzzyAssignments.contains(entry)) {
							...
							if (context != null) {
								...
								con.add(depiction, f.createURI(Constants.mainNS+"assignedFrom"), f.createURI(Constants.mainNS+"fuzzyAndGeoRules"), context);
								...
								con.add(depiction, f.createURI(Constants.mainNS+"assignedFrom"), f.createURI(Constants.mainNS+"fuzzyAndGeoRules"));
								con.add(depiction, RDF.TYPE, f.createURI(Constants.mainNS+"Depiction"));
							}
						}
						else { // if the geo rule is not evaluated, then delete the fuzzy assignment
							String q = "SELECT ?dep WHERE {"+
										"<"+entry.stringValue()+"> gno:"+Constants.depicts+" ?dep . \n"+
										"?dep gno:"+Constants.depictsObject+" <"+rule.getHead().getRegionDepiction()+"> . \n"+
										"}";
							...
							while (result.hasNext()) {
								...
								if (context == null) {
									con.remove(entry, Constants.depictsProperty, (URI)bindingSet.getValue("dep"));
									con.remove((URI)bindingSet.getValue("dep"), Constants.depictsObjectProperty, rule.getHead().getRegionDepiction());
									con.remove((URI)bindingSet.getValue("dep"), Constants.withConfidenceProperty, null);
								}
								else {
									con.remove(entry, Constants.depictsProperty, (URI)bindingSet.getValue("dep"), context);
									con.remove((URI)bindingSet.getValue("dep"), Constants.depictsObjectProperty, rule.getHead().getRegionDepiction(), context);
									con.remove((URI)bindingSet.getValue("dep"), Constants.withConfidenceProperty, null, context);
								}
							}
						}
						con.commit();
				...
				while (result.hasNext()) {
					...
					if (counter > 0) {
						...
						if (context == null) {
							...
							con.remove(depiction, f.createURI(Constants.mainNS+"assignedFrom"), f.createURI(Constants.mainNS+"fuzzyAndGeoRules"));
							...
							con.remove(depiction, f.createURI(Constants.mainNS+"assignedFrom"), f.createURI(Constants.mainNS+"fuzzyAndGeoRules"), context);
	...
	public Map<URI,Literal> fire(GeoRule rule, DataSource ds, RepositoryConnection con ) {
		...
		for (GeoRulePredicate grp : rule.getPredicates()) {
			if (firstPredicate) {
				uris = geoQueryAssigned(grp, rule.getHead(), con);
	...
	public List<URI> getAssignedList(GeoRuleHead headClause, RepositoryConnection con) {
		List<URI> ret = new ArrayList<URI>();
		String q = "SELECT ?reg WHERE {\n" +
				"?reg gno:"+Constants.depicts+" ?dep . \n" +
				"?dep gno:"+Constants.depictsObject+" ?obj . \n"+
				"?obj rdf:type <"+headClause.getRegionDepiction().stringValue()+"> . \n"+
				"FILTER NOT EXISTS {" +
				"?reg2 gno:"+Constants.depicts+" ?dep2 . \n"+
				"?dep2 gno:assignedFrom gno:geoRule . \n"+
				"}"+
				"FILTER NOT EXISTS {" +
				"?reg2 gno:"+Constants.depicts+" ?dep2 . \n"+
				"?dep2 gno:assignedFrom gno:fuzzyAndGeoRule . \n"+
				"}"+
			"}";
		try {
			TupleQuery regTupleQuery = con.prepareTupleQuery(QueryLanguage.SPARQL, Constants.queryPrefix+q);
			TupleQueryResult regResult = regTupleQuery.evaluate();
			while (regResult.hasNext()) {
				ret.add((URI)regResult.next().getValue("reg"));
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Map<URI,Literal> geoQueryAssigned(GeoRulePredicate grp, GeoRuleHead headClause, RepositoryConnection con) {
		...
		try {
			...
			String reg2Query = "SELECT ?reg2 ?reg2WKT WHERE {\n" +
									...
									"?reg2 gno:"+Constants.depicts+" ?dep2 . \n" +
									"?dep2 gno:"+Constants.depictsObject+" ?obj . \n"+
									"?obj2 rdf:type <"+headClause.getRegionDepiction().stringValue()+"> . \n"+
									"FILTER (?reg != ?reg2) . \n" +
									"FILTER NOT EXISTS {" +
										"?dep2 gno:assignedFrom gno:geoRule . "+
									"} . \n"+
